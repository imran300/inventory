/*
 * activeit ADMIN TEMPLATE JAVASCRIPT
 * ======================================================================
...
For more efficient, simply called "activeit.container".
...
!function ($) {
	...
	activeit.window.on('load', function(){
		...
		activeit.body.addClass('activeit-ready');
	});
...
!function ($) {
	...
	activeit.window.one('load', function(){
		if (activeit.scrollTop.length && !activeit.isMobile) {
			var isVisible = true;
			var offsetTop = 250;

			activeit.window.scroll(function(){
				if (activeit.window.scrollTop() > offsetTop && !isVisible) {
					activeit.navbar.addClass('shadow');
					activeit.scrollTop.addClass('in');
					isVisible = true;
				}else if (activeit.window.scrollTop() < offsetTop && isVisible) {
					activeit.navbar.removeClass('shadow');
					activeit.scrollTop.removeClass('in');
					isVisible = false;
				}
			});

			activeit.scrollTop.on('click', function(e){
				e.preventDefault();

				activeit.bodyHtml.animate({
					scrollTop : 0
				}, 500);
			});
...
!function ($) {
	...
	methods = {
		'show' : function(el){
			...
			ovId = 'activeit-overlay-' + uID() + uID()+"-" + uID(),
			panelOv = $('<div id="'+ ovId +'" class="panel-overlay"></div>');

			el.prop('disabled', true).data('activeitOverlay',ovId);
		...
		'hide': function(el){
			var target = $(el.attr('data-target'));
			var boxLoad = $('#'+ el.data('activeitOverlay'));
	...
	$.fn.activeitOverlay = function(method){
		if (methods[method]){
...
!function ($) {
	...
	$.activeitNoty = function(options){
		...
		focusElement = function(pos){
			activeit.bodyHtml.animate({scrollTop: pos}, 300, function(){
				el.addClass('in');
			});
		},
		init = function(){
			if (opt.container === 'page') {
				if (!pageHolder) {
					...
					activeit.contentContainer.prepend(pageHolder);
				};
				...
				if (!floatContainer[opt.floating.position]) {
					...
					activeit.container.append(floatContainer[opt.floating.position]);
				}
...
!function ($) {
	...
	formElement = function(el){
		if (el.data('activeit-check')) return
		else
		el.data('activeit-check', true)
		...
		changed = function(){
			if(input.type == 'radio' && $groupInput.length) {
				$groupInput.each(function(){
					var $gi = $(this);
					if ($gi.hasClass('active')) $gi.trigger('activeit.ch.unchecked');
			...
			if (input.checked) {
				el.addClass('active').trigger('activeit.ch.checked');
			}else{
				el.removeClass('active').trigger('activeit.ch.unchecked');
			}
		};
	...
	$.fn.activeitCheck = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
	...
	activeit.document.ready(function() {
		allFormEl = $('.form-checkbox, .form-radio');
		if(allFormEl.length) allFormEl.activeitCheck();
	});

	activeit.document.on('change', '.btn-file :file', function() {
		var input = $(this),
...
!function ($) {
		...
		$menulink.each(function(){
			...
			on('shown.bs.popover', function () {
				...
				offsetBottom 	= function(){
					if(activeit.container.hasClass('mainnav-fixed')){
				...
				if( offsetTop > offsetBottom){
					...
					if(!activeit.container.hasClass('navbar-fixed') && activeit.mainNav.hasClass('affix-top')) offsetBottom -= 50;
					if(popoverHeight > offsetBottom){
						if(activeit.container.hasClass('navbar-fixed') || activeit.mainNav.hasClass('affix-top')) offsetBottom -= (elHeight + 5);
			...
			.on('click', function(){
				if(!activeit.container.hasClass('mainnav-sm')) return;
	...
	updateSize = function(){
		...
		var sw = activeit.container.width(), currentScreen;
		...
		if (screenCat != currentScreen){
			...
			activeit.screenSize = currentScreen;

			if(activeit.screenSize == 'sm' && activeit.container.hasClass('mainnav-lg')){
	...
	updateNav = function(e){
		...
		if (updateMethod == 'collapse' || activeit.container.hasClass('mainnav-sm') ) {
			activeit.container.removeClass('mainnav-in mainnav-out mainnav-lg');
			bindSmallNav();
		}
	...
	init = function(){
		if (!defaultSize) {
			defaultSize = {
				xs : 'mainnav-out',
				sm : activeit.mainNav.data('sm') || activeit.mainNav.data('all'),
				md : activeit.mainNav.data('md') || activeit.mainNav.data('all'),
				lg : activeit.mainNav.data('lg') || activeit.mainNav.data('all')
			}
	...
	methods = {
		'revealToggle' : function(){
			if (!activeit.container.hasClass('reveal')) activeit.container.addClass('reveal');
			activeit.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealIn' : function(){
			if (!activeit.container.hasClass('reveal')) activeit.container.addClass('reveal');
			activeit.container.addClass('mainnav-in').removeClass('mainnav-out mainnav-lg mainnav-sm');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealOut' : function(){
			if (!activeit.container.hasClass('reveal')) activeit.container.addClass('reveal');
			activeit.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'slideToggle' : function(){
			if (!activeit.container.hasClass('slide')) activeit.container.addClass('slide');
			activeit.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		...
		'pushToggle' : function(){
			activeit.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm');
			if (activeit.container.hasClass('mainnav-in mainnav-out')) activeit.container.removeClass('mainnav-in');
			//if (activeit.container.hasClass('mainnav-in')) //activeit.container.removeClass('aside-in');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'pushIn' : function(){
		...
		'colExpToggle' : function(){
			if (activeit.container.hasClass('mainnav-lg mainnav-sm')) activeit.container.removeClass('mainnav-lg');
			activeit.container.toggleClass('mainnav-lg mainnav-sm').removeClass('mainnav-in mainnav-out');
			return activeit.window.trigger('resize');
		},
		'collapse' : function(){
			activeit.container.addClass('mainnav-sm').removeClass('mainnav-lg mainnav-in mainnav-out');
			updateMethod = 'collapse';
			return activeit.window.trigger('resize');
		},
		'expand' : function(){
			activeit.container.removeClass('mainnav-sm mainnav-in mainnav-out').addClass('mainnav-lg');
			return activeit.window.trigger('resize');
		},
		'togglePosition' : function(){
			activeit.container.toggleClass('mainnav-fixed');
			activeit.mainNav.activeitAffix('update');
		},
		'fixedPosition' : function(){
			activeit.container.addClass('mainnav-fixed');
			activeit.mainNav.activeitAffix('update');
		},
		'staticPosition' : function(){
			activeit.container.removeClass('mainnav-fixed');
			activeit.mainNav.activeitAffix('update');
		},
		'update' : updateNav,
	...
	$.activeitNav = function(method,complete){
		if (methods[method]){
			if(method == 'colExpToggle' || method == 'expand' || method == 'collapse'){
				if(activeit.screenSize == 'xs' && method == 'collapse'){
					method = 'pushOut';
				}else if((activeit.screenSize == 'xs' || activeit.screenSize == 'sm') && (method=='colExpToggle' || method == 'expand') && activeit.container.hasClass('mainnav-sm')){
					method = 'pushIn';
				}
			}
	...
	$.fn.isOnScreen = function(){
		var viewport = {
			top : activeit.window.scrollTop(),
			left : activeit.window.scrollLeft()
		};
		viewport.right = viewport.left + activeit.window.width();
		viewport.bottom = viewport.top + activeit.window.height();
	...
	activeit.window.on('resizeEnd',updateNav).trigger('resize');
	...
	activeit.window.on('load', function() {
		...
		if(toggleBtn.length){
			toggleBtn.on('click', function(e){
				...
				if(toggleBtn.hasClass('push')){
				 	 $.activeitNav('pushToggle');
				}else if(toggleBtn.hasClass('slide')){
					$.activeitNav('slideToggle');
				}else if(toggleBtn.hasClass('reveal')){
					$.activeitNav('revealToggle');
				}else{
		...
		if (menu.length) {
			...
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			activeit.mainNav.find('.nano').nanoScroller({
				preventPageScrolling : true
			});
...
!function ($) {
	...
	var asideMethods = {
		'toggleHideShow' : function(){
			activeit.container.toggleClass('aside-in');
			activeit.window.trigger('resize');
			if(activeit.container.hasClass('aside-in')){
				toggleNav();
			}
		},
		'show' : function(){
			activeit.container.addClass('aside-in');
			activeit.window.trigger('resize');
			toggleNav();
		},
		'hide' : function(){
			activeit.container.removeClass('aside-in');
			activeit.window.trigger('resize');
		},
		'toggleAlign' : function(){
			activeit.container.toggleClass('aside-left');
			activeit.aside.activeitAffix('update');
		},
		'alignLeft' : function(){
			activeit.container.addClass('aside-left');
			activeit.aside.activeitAffix('update');
		},
		'alignRight' : function(){
			activeit.container.removeClass('aside-left');
			activeit.aside.activeitAffix('update');
		},
		'togglePosition' : function(){
			activeit.container.toggleClass('aside-fixed');
			activeit.aside.activeitAffix('update');
		},
		'fixedPosition' : function(){
			activeit.container.addClass('aside-fixed');
			activeit.aside.activeitAffix('update');
		},
		'staticPosition' : function(){
			activeit.container.removeClass('aside-fixed');
			activeit.aside.activeitAffix('update');
		},
		'toggleTheme' : function(){
			activeit.container.toggleClass('aside-bright');
		},
		'brightTheme' : function(){
			activeit.container.addClass('aside-bright');
		},
		'darkTheme' : function(){
			activeit.container.removeClass('aside-bright');
		}
	},
	toggleNav = function(){
		if(activeit.container.hasClass('mainnav-in')){
			activeit.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
		}
	};
	...
	$.activeitAside = function(method,complete){
	...
	activeit.window.on('load', function() {
		if(activeit.aside.length){
			// STYLEABLE SCROLLBARS
			// =================================================================
			// Require nanoScroller
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			activeit.aside.find('.nano').nanoScroller({
				preventPageScrolling : true,
				alwaysVisible: false
			});
...
/* ========================================================================
* activeit LANGUAGE SELECTOR v1.0
* -------------------------------------------------------------------------
...
!function ($) {
	...
	$.fn.activeitLanguage = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
...
!function ($) {
	...
	$.fn.activeitAffix = function(method){
		return this.each(function(){
			var el = $(this), className;

			if (typeof method === 'object' || !method){
				className = method.className;
				el.data('activeit.af.class', method.className);
			}else if (method == 'update') {
				className = el.data('activeit.af.class');
			}

			if (activeit.container.hasClass(className) && !activeit.container.hasClass('navbar-fixed') ) {
				el.affix({
					offset:{
					top:$('#navbar').outerHeight()
					}
				});
			}else if (!activeit.container.hasClass(className) || activeit.container.hasClass('navbar-fixed')) {
				activeit.window.off(el.attr('id') +'.affix');
	...
	activeit.window.on('load', function(){
		if (activeit.mainNav.length) {
			activeit.mainNav.activeitAffix({className : 'mainnav-fixed'});
		}

		if (activeit.aside.length) {
			activeit.aside.activeitAffix({className : 'aside-fixed'});
		}
	});
